  ...
  if (servoValuesFile) {
    ...
    while (servoValuesFile.available()) {
      if ( index <= SERVO_COUNT) {
        ...
        ServoCalibPose[index] = servoValue;
...
void logServoValues() {
  ...
  if (servoValuesFile) {
    ...
    for (int servoI = 0; servoI < SERVO_COUNT; servoI++) {
      calibString += String(ServoCalibPose[servoI]) + ",";
